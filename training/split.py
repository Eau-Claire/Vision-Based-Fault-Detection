...
# 1. Cấu hình đường dẫn (Đảm bảo đúng thư mục chứa file _classes.csv mới)
BASE_DIR = "../dataset/train" 
CSV_FILE = os.path.join(BASE_DIR, "_classes.csv")

# 2. Cập nhật danh sách nhãn mới từ ảnh Roboflow của bạn
# Mình bỏ qua 'insulator' và 'null' nếu bạn chỉ muốn tập trung vào lỗi cụ thể
CLASSES = [
    'broken disc', 
    'broken glass', 
    'pollution-flashover'
...
# 4. Tạo các thư mục nhãn mới
for cls in CLASSES:
    os.makedirs(os.path.join(BASE_DIR, cls), exist_ok=True)

# 5. Duyệt qua từng hàng trong CSV để di chuyển ảnh
print("--- Đang bắt đầu phân loại các lỗi sứ mới ---")
for index, row in df.iterrows():
    file_name = row['filename'].strip()
    src_path = os.path.join(BASE_DIR, file_name)
    
    # Kiểm tra ảnh thuộc nhãn lỗi nào
    for cls in CLASSES:
        # Nếu cột đó bằng 1 thì di chuyển vào folder tương ứng
...
print("Xong! Bây giờ cấu trúc folder của Châu đã có thêm các loại lỗi mới.")